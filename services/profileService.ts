...
class ProfileService {
    ...
    /**
     * Accept terms and privacy policy, including optional custom partnership interest
     */
    async acceptTerms(userId: string, customInterest: string | null): Promise<void> {
        try {
    ...
    // ============================================
    // BENEFIT MODERATION (Admin)
    // ============================================

    /**
     * Get all profiles with pending benefits (Admin only)
     * Only returns benefits that have real content (title + description filled)
     */
    async getPendingBenefits(): Promise<ProfileData[]> {
        try {
            const { data, error } = await supabase
                .from('profiles')
                .select('id, name, email, image_url, company, job_title, role, exclusive_benefit, benefit_status, created_at, updated_at')
                .eq('benefit_status', 'pending')
                .not('exclusive_benefit', 'is', null)
                .neq('exclusive_benefit->>title', '')
                .neq('exclusive_benefit->>description', '')
                .order('updated_at', { ascending: false });

            if (error) throw error;
            // Extra safety: filter client-side for non-empty titles
            const filtered = (data || []).filter(p =>
                p.exclusive_benefit?.title?.trim() && p.exclusive_benefit?.description?.trim()
            );
            return filtered as ProfileData[];
        } catch (error) {
            logger.error('Error fetching pending benefits:', error);
            return [];
        }
    }

    /**
     * Update benefit moderation status (Admin only)
     * Supports adding rejection_reason and triggering notification
     */
    async updateBenefitStatus(userId: string, newStatus: 'approved' | 'rejected', rejectionReason?: string): Promise<boolean> {
        try {
            ...
            if (newStatus === 'rejected') {
                ...
                updatePayload.rejection_reason = null; // Clean if approved
            }
            ...
            // Trigger IN-APP Notification asynchronously if rejected
            if (newStatus === 'rejected') {
                // Dynamically import to avoid circular dependency issues based on component trees
                import('./notificationService').then(({ notificationService }) => {
                    notificationService.createNotification(
                        "Oferta de Benefício Recusada",
                        "Sua oferta de benefício precisa de ajustes. Toque aqui para ver as observações da Diretoria.",
                        'INDIVIDUAL',
                        "/profile",
                        userId
                    ).catch(err => logger.error('Failed to send rejection notification:', err));
                });
            }

            return true;
        } catch (error) {
            logger.error(`Error updating benefit status to ${newStatus}:`, error);
            return false;
        }
    }

    /**
     * Delete benefit completely (Member action for resetting rejected benefit)
     */
    async deleteBenefit(userId: string): Promise<boolean> {
        try {
            const { error } = await supabase
                .from('profiles')
                .update({ 
                    exclusive_benefit: null, 
                    benefit_status: null, 
