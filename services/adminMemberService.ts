...
class AdminMemberService {
    ...
    async getMembers(
        ...
        filters?: {
            search?: string;
            role?: 'ALL' | 'ADMIN' | 'CEO' | 'MANAGER' | 'ACCOUNT_MANAGER' | 'TEAM' | 'MEMBER';
            fetchAll?: boolean; // When true, skip pagination (for client-side filtering)
        }
        ...
        // Server-side search filter (name, email, company)
        if (filters?.search && filters.search.trim()) {
            const term = filters.search.trim();
            query = query.or(`name.ilike.%${term}%,email.ilike.%${term}%,company.ilike.%${term}%`);
        }

        // Deterministic sort: primary + tie-breaker (id) to prevent PostgreSQL page shuffle
        query = query
            .order('created_at', { ascending: false })
            .order('id', { ascending: false });

        // Only apply range when NOT fetching all (server-side pagination)
        if (!filters?.fetchAll) {
