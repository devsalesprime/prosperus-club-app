...
export function useProfileForm({ currentUser, supabase, isMockMode, onSave }: UseProfileFormParams) {
    ...
    useEffect(() => {
        // Força a atualização do cache do perfil ao abrir o modal
        // Isso resolve o problema de administradores testarem aprovações/recusas na mesma sessão
        // sem precisar dar F5 no navegador.
        if (currentUser?.id) {
            refreshProfile().catch(err => logger.warn('Background profile refresh ignored:', err));
        }
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            const cleanedFormData = {
                ...
                // Se o benefício está ativo, forçar status para 'pending' (moderação)
                ...(formData.exclusive_benefit?.active ? { benefit_status: 'pending' as const } : {})
            };
            ...
            if (updatedProfile) {
                ...
                notify.success(hasBenefitPending
                    ? 'Perfil atualizado. Benefício enviado para análise e aprovação da Diretoria.'
                    : 'Perfil atualizado.');
                setTimeout(() => {
