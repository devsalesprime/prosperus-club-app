...
export async function registerPushNotifications(userId: string): Promise<boolean> {
    try {
        ...
        if (!('Notification' in window)) {
            logger.warn('Notifications não suportadas neste navegador');
            return false;
        }
        ...
        if (!('PushManager' in window)) {
            logger.warn('Push API não suportada neste navegador');
            return false;
        }
        ...
        if (permission !== 'granted') {
            logger.warn('Permissão de notificações negada pelo usuário');
            return false;
        }
        ...
        if (!subscription) {
            ...
            if (!vapidPublicKey) {
                logger.error('VITE_VAPID_PUBLIC_KEY não configurada no .env');
                return false;
            }
        ...
        if (error) {
            logger.error('Erro ao salvar dispositivo no banco:', error);
            return false;
        }
...
export async function unregisterPushNotifications(userId: string): Promise<boolean> {
    try {
        ...
        if (subscription) {
            ...
            if (error) {
                logger.error('Erro ao remover dispositivo do banco:', error);
                return false;
            }
        ...
        logger.error('Erro ao desregistrar push notifications:', error);
        return false;
    }
}
