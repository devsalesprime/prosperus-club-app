            ...
            >
                ...
                <div className="flex items-center text-slate-400 text-sm">
                    {event.category === 'PRESENTIAL' ? (
                        ...
                        <Calendar size={16} className="mr-1.5 shrink-0 text-orange-400" />
                    )}
...
const ListCard: React.FC<{ event: ClubEvent; onClick: () => void }> = ({ event, onClick }) => {
    ...
    return (
        <div
            onClick={onClick}
            className={`mb-3 bg-slate-900 border-l-4 ${config.borderLeft} rounded-r-lg overflow-hidden cursor-pointer transition-all hover:shadow-lg hover:shadow-yellow-500/10 hover:border-yellow-500`}
        >
            <div className="flex">
                {/* Time Column */}
                <div className="flex-shrink-0 w-20 flex flex-col items-center justify-center bg-slate-800 p-3">
                    <Clock size={16} className="text-slate-400 mb-1" />
                    <div className="text-sm font-bold text-white">{eventTime}</div>
                </div>

                {/* Event Content */}
                <div className={`flex-1 p-4 ${config.bg}`}>
                    ...
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                        <span className={`inline-flex items-center gap-1.5 px-2 py-1 text-xs font-bold ${config.bg} ${config.text} border ${config.border}`}>
                            {config.icon}
                            {config.label}
                        </span>
                        {isMultiDay && (
                            <span className="inline-flex items-center gap-1.5 px-2 py-1 text-xs font-bold bg-orange-500/10 text-orange-400 border border-orange-500/30">
