...
export const ProfileEdit: React.FC<ProfileEditProps> = ({ currentUser, supabase, isMockMode = false, onSave, onCancel }) => {
    ...
    // Reset rejected benefit
    const handleResetBenefit = async () => {
        try {
            await profileService.deleteBenefit(currentUser.id);
            ...
            // Reset status locally to re-enable the form
            currentUser.benefit_status = undefined;
            currentUser.rejection_reason = undefined;
            
            // Background update
            refreshProfile();

            notify.success('Sua oferta anterior foi excluída. Você pode cadastrar uma nova!');
        } catch(err) {
            notify.error('Não foi possível limpar a oferta agora.');
        }
    };
                ...
                <ProfilePhotoEditor
                    ...
                    onConfirm={async (croppedBlob) => {
                        ...
                        try {
                            ...
                            await profileService.updateProfile(currentUser.id, { image_url: publicUrl } as Partial<any>);
                    ...
                    onRemove={async () => {
                        try {
                            await profileService.updateProfile(currentUser.id, { image_url: null } as Partial<any>);
