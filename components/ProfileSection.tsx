...
import {
    ...
    Linkedin, Instagram, Phone, Globe, HelpCircle, Cake, AlertTriangle
} from 'lucide-react';
...
const ProfileSection: React.FC<ProfileSectionProps> = ({
    ...
    const { tour } = useApp();
    const [hideRejectionAlert, setHideRejectionAlert] = React.useState(false);
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in relative">
            ...
            {/* Rejection Alert (If Benefit Rejected) */}
            {currentUser.benefit_status === 'rejected' && !hideRejectionAlert && (
                <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-6 shadow-lg shadow-red-900/10 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-start gap-3">
                        <div className="p-2 bg-red-500/20 rounded-lg shrink-0 mt-0.5">
                            <AlertTriangle size={20} className="text-red-500" />
                        </div>
                        <div>
                            <h3 className="text-red-500 font-bold mb-1">Atenção: Sua oferta para o clube precisa de ajustes</h3>
                            <p className="text-sm text-slate-300 mb-3">
                                A sua oferta de benefício foi avaliada pela moderação e precisa ser ajustada antes de ir para o diretório de sócios.
                            </p>
                            
                            {currentUser.rejection_reason && (
                                <div className="text-sm bg-black/40 p-3 rounded-lg border border-red-500/20 italic text-slate-200 mb-4">
                                    "{currentUser.rejection_reason}"
                                </div>
                            )}
                            ...
                            <button
                                onClick={() => {
                                    setHideRejectionAlert(true);
                                    setIsEditingProfile(true);
                                }}
                                className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-500 text-white font-bold rounded-lg transition"
                            >
                                <Settings size={18} />
                                Corrigir Oferta Agora
                            </button>
                        </div>
                    </div>
                </div>
