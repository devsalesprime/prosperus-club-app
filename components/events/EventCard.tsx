...
const HeroCard: React.FC<{ event: ClubEvent; onClick: () => void; onViewAgenda?: () => void }> = ({
    ...
    return (
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700 overflow-hidden flex flex-col md:flex-row hover:border-yellow-600/50 transition-all group shadow-xl hover:shadow-2xl hover:shadow-yellow-600/10">
            {/* Gold Date Block */}
            <div
                className="md:w-32 bg-gradient-to-br from-prosperus-gold-light to-prosperus-gold-dark p-6 flex flex-col items-center justify-center text-center shrink-0 relative overflow-hidden cursor-pointer"
                onClick={onClick}
            ...
            >
                ...
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    {/* Countdown Badge */}
                    {timeDiff > 0 && (
                        <span className={`px-2.5 py-1 ${isToday ? 'bg-yellow-500 text-slate-900 animate-pulse' : 'bg-yellow-500/10 text-prosperus-gold border border-yellow-500/30'} text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5`}>
                            <Sparkles size={12} className={isToday ? 'text-slate-900' : 'text-prosperus-gold'} />
                ...
                {/* Title */}
                <h3 className="text-xl font-black text-white mb-2 line-clamp-2 group-hover:text-prosperus-gold transition-colors leading-tight">
                    {event.title}
                </h3>
            ...
            {onViewAgenda && (
                ...
                >
                    <div className="flex flex-col items-center gap-2 text-prosperus-gold hover:text-yellow-400 transition-colors">
        ...
        >
            <div className="flex">
                ...
                <div className="flex-1 p-4 bg-slate-800/50">
                    <h3 className="text-base font-bold text-white mb-2 line-clamp-2 group-hover:text-prosperus-gold transition-colors">
                        {event.title}
                    </h3>
                    ...
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                        ...
                        {timeDiff > 0 && (
                            <span className={`inline-flex items-center gap-1.5 px-2 py-1 text-[10px] font-black uppercase tracking-wider ${isToday ? 'bg-yellow-500 text-slate-900 animate-pulse' : 'bg-yellow-500/10 text-prosperus-gold border border-yellow-500/30'}`}>
                                <Sparkles size={12} className={isToday ? 'text-slate-900' : 'text-prosperus-gold'} />
                                {isToday ? 'É HOJE!' : `Faltam ${days > 0 ? `${days}d ` : ''}${hours}h`}
                            </span>
                        )}
