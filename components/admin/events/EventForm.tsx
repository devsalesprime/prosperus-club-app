...
/** 
 * Converte uma string de datetime-local (sem timezone) para ISO 8601 com offset de Brasília.
 * Ex: "2026-03-30T09:00" → "2026-03-30T09:00:00-03:00"
 * Isso garante que o Supabase (timestamptz) armazene a hora correta.
 */
const toBrasiliaISO = (localDatetime: string): string => {
    if (!localDatetime) return localDatetime;
    // Se já tem timezone info (Z, +, -), retorna como está
    if (/[Z+]/.test(localDatetime.slice(10)) || localDatetime.includes('-', 11)) {
        // Verifica se o '-' após posição 10 é de timezone e não parte da data
        const timePart = localDatetime.slice(10);
        if (timePart.includes('Z') || timePart.includes('+') || (timePart.match(/-/g) || []).length > 0) {
            // Pode já ter offset, verificar mais precisamente
            if (/[+-]\d{2}:\d{2}$/.test(localDatetime) || localDatetime.endsWith('Z')) {
                return localDatetime;
            }
        }
    }
    // Formato datetime-local: "2026-03-30T09:00" → adiciona segundos e offset BRT
...
export const EventForm: React.FC<EventFormProps> = ({ event, onSaved, onCancel }) => {
    ...
    const onSubmit = async (data: any) => {
        try {
            ...
            const eventData = {
                ...data,
                // Converter datas locais para ISO com offset de Brasília
                date: toBrasiliaISO(data.date),
                endDate: data.endDate ? toBrasiliaISO(data.endDate) : undefined,
                materials,
            ...
            if (hasMultipleSessions && sessions.length > 0) {
                ...
                eventData.date = toBrasiliaISO(`${first.date}T${first.startTime}`);
                eventData.endDate = toBrasiliaISO(`${last.date}T${last.endTime}`);
            }
